...
async function getPerformances(request: NextRequest) {
  try {
    ...
    return handleServerError(error, 'Erreur lors de la récupération des performances')
  }
}
...
async function createPerformance(request: NextRequest) {
  try {
    ...
    const userId = payload.role === 'ARTIST' ? payload.id : body.userId || payload.id
    ...
    return handleServerError(error, 'Erreur lors de la création de la performance')
  }
}

export const POST = withRateLimit(createPerformance, { windowMs: 60 * 1000, max: 10 })
