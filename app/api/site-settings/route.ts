...
async function getSiteSettings(request: NextRequest) {
  try {
    ...
    if (key) {
      const fallbackValue =
        fallbackSiteSettings[key as keyof typeof fallbackSiteSettings];
      if (fallbackValue) {
        return NextResponse.json({ setting: fallbackValue }, { status: 200 });
      }
      return NextResponse.json({ setting: null }, { status: 200 });
    }

    // Convertir les fallback settings en format attendu
    const settings = Object.entries(fallbackSiteSettings).map(
      ([key, setting]) => ({
        key,
        ...setting,
      }),
...
export const POST = withRateLimit(createOrUpdateSetting, {
  windowMs: 60 * 1000,
  max: 10,
});
