    ...
    async function loadEvent() {
      try {
        ...
        if (!res.ok) {
          ...
          console.error('Error loading event:', res.status, errorText)
          setError(`Erreur lors du chargement: ${res.status === 404 ? 'Événement non trouvé' : 'Erreur serveur'}`)
          setLoading(false)
          // Rediriger seulement après un délai pour permettre à l'utilisateur de voir l'erreur
          setTimeout(() => {
        ...
        if (!data.event) {
          setError('Événement non trouvé dans la réponse')
          setLoading(false)
          setTimeout(() => {
            router.push('/admin/events')
          }, 3000)
        ...
        setFormData({
          ...
          ticketPrice: event.ticketPrice !== null && event.ticketPrice !== undefined ? event.ticketPrice.toString() : '',
    ...
    <div className="container mx-auto px-4 py-8 md:py-16">
      ...
      >
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <h1 className="text-3xl md:text-4xl font-bold">Modifier l&apos;événement</h1>
