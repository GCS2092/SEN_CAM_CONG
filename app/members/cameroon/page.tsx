...
export default function CameroonMembersPage() {
  ...
  useEffect(() => {
    async function fetchMembers() {
      try {
        const res = await fetch('/api/members?nationality=Cameroun&active=true')
              ...
              >
                <div className="p-8">
                  <div className="flex flex-col md:flex-row gap-6">
                    <div className="relative w-full md:w-48 h-48 rounded-2xl overflow-hidden flex-shrink-0 shadow-lg group-hover:shadow-xl transition-shadow bg-gradient-to-br from-cameroon-red/20 to-cameroon-red/10">
                      {member.imageUrl ? (
                        <Image
                          src={member.imageUrl}
                          alt={member.name}
